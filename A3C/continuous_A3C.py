...
os.environ["OMP_NUM_THREADS"] = "1"        # 设置使用的线程数量， 因为 pytorch 默认使用全部的线程
...
class Net(nn.Module):
    ...
    def choose_action(self, s):
        ...
        m = self.distribution(mu.view(1, ).data, sigma.view(1, ).data)     # Normal(loc ： 均值（mu）, scale ： 标准差（sigma）)
    ...
    def loss_func(self, s, a, v_t):
        ...
        entropy = 0.5 + 0.5*math.log(2*math.pi) + T.log(m.scale)          # exploration
...
if __name__ == "__main__":
    ...
    gnet.share_memory()          # share the global parameters in multiprocessing 在多处理中共享全局参数
    opt = SharedAdam(gnet.parameters(), lr=1e-4, betas=(0.95, 0.999))    # global optimizer
    global_ep, global_ep_r, res_queue = mp.Value('i', 0), mp.Value('d', 0.), mp.Queue()

    # parallel training  平行训练
    workers = [Worker(gnet, opt, global_ep, global_ep_r, res_queue, i) for i in range(mp.cpu_count())]       # Worker的集合
    [w.start() for w in workers]
    res = []     # record episode reward to plot 记录情节奖励以作图
    while True:
        r = res_queue.get()
        if r is not None:
            res.append(r)

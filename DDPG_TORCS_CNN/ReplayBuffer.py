...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim , memory_size , batch_size = 32, act_dim = 3):
        ...
        self.obs_buf = np.zeros([memory_size, 3, self.image_size_w, self.image_size_h], dtype = np.float32)
        self.next_obs_buf = np.zeros([memory_size, 3, self.image_size_w, self.image_size_h], dtype = np.float32)
